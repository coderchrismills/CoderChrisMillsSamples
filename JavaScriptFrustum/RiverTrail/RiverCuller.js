(function() {
	RiverCuller = function(size) {
		var initPos = new Array(size);
		var initRes = new Array(size);

		for (var i=0; i<initPos.length; i++) {
			initPos[i] = new ParallelArray([Math.random()*2, Math.random()*2, Math.random()*2, 1]);
			initRes[i] = 0.0;
		}

		this.pos = new ParallelArray(Float32Array, initPos);
		this.res = new ParallelArray(initRes);

		this.frustum = [
			new ParallelArray([+1, 0, 0, 2]),
			new ParallelArray([-1, 0, 0, 2]),
			new ParallelArray([0, +1, 0, 2]),
			new ParallelArray([0, -1, 0, 2]),
			new ParallelArray([0, 0, +1, 2]),
			new ParallelArray([0, 0, -1, 2])
		];
	}	
	...
	RiverCuller.prototype.cullTest = function(iter) {
		var start = Date.now();
